...
function connect() {
  ...
  const maybeUp = () => { if (streamUp && ctlUp) { setConn(true); toast("● 双通道已建立 (" + S.videoMode + ")"); startPing(); pollStatus(); } };

  const sws = new WebSocket(wsBase() + "/ws/stream" + (token ? "?token=" + encodeURIComponent(token) : ""));
  sws.binaryType = "arraybuffer";
  S.streamWs = sws;
  sws.onopen = () => { streamUp = true; maybeUp(); };
  sws.onmessage = ev => {
  ...
  sws.onclose = () => { if (S.streamWs === sws) { setConn(false); showBoot("推流中断，点击重连"); $("bootOverlay").classList.remove("hide"); } };
  sws.onerror = () => { try { sws.close(); } catch (e) {} };

  const cws = new WebSocket(wsBase() + "/ws/control" + (token ? "?token=" + encodeURIComponent(token) : ""));
  S.ctlWs = cws;
  cws.onopen = () => { ctlUp = true; maybeUp(); declareVideoMode(); if (!S.audioEnabled) sendCtl({ type: "audio", enabled: false }); };
  cws.onmessage = ev => {
    let m = null;
  ...
  cws.onclose = () => { stopPing(); if (S.ctlWs === cws) setConn(false); };
